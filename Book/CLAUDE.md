...
└── projects/
    ├── lighthouse.html          ← 1º case real (Vale/Lighthouse), bilíngue, a partir do template. Imagens em images/lighthouse/
    ...
    ├── case-template.js         ← interações: reveal+stagger, marginalia, footnotes, lupa, progress, page-counter, i18n PT/EN, tilt 3D, parallax, count-up
    ├── calligraphy-demo.html    ← demo isolado de caligrafia traçada (avaliar antes de aplicar no Frontispiece)
    └── motion-3d-demo.html      ← lab isolado de 3D/motion (4 efeitos; só ii=tilt e iv=motion foram aplicados ao template)
```
...
- ATENÇÃO ao sandbox: o mount do bash às vezes serve cópias **truncadas** dos .js/.css editados via ferramenta de arquivo; validar no arquivo host. O Playwright headless lê o mount, então features no fim dos arquivos podem falhar no teste mas funcionam no host.
